...
const CargarTodoTipoArchivo = ({ getListaArchivos, setListaArchivos }) => {
  ...
  const handleDrop = acceptedFiles => {
    ...
    if (updatedFiles.length + acceptedFiles.length > maxFiles) {
      Swal.fire('Advertencia', `${t('No puedes subir más de')} ${maxFiles} ${t('archivos')}`, 'warning')
    ...
    acceptedFiles.forEach(file => {
      ...
      if (existingFileIndex !== -1) {
        Swal.fire(t('Advertencia'), `${'El archivo'} ${file.name} ${'ya existe'}`, 'question')
      } else {
      ...
      >
        ...
        <Grid style={{ marginTop: '10px' }}>
          <Typography color='textSecondary'>
            Arrastra los archivos aquí o haz clic para navegar en tu dispositivo
          </Typography>
...
export default CargarTodoTipoArchivo
