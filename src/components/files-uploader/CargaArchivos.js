...
const CargaArchivos = ({
  ...
  maxFiles,
  archivosAceptados = ['*'],
  mensaje = 'Solo puede subir imágenes y archivos .pdf',
  maxSize = 20971520
}) => {
  // let maxFiles = 20 // Cambiar la cantidad máxima de archivos permitidos aquí, ya está automatizado
  // para este componente 1mb el equivalente a 1048576
  ...
  const allowedExtensions = Object.values(archivosAceptados).flat()
  ...
  const handleDrop = acceptedFiles => {
    ...
    if (updatedFiles.length + acceptedFiles.length > maxFiles) {
      Swal.fire(
        t('Advertencia'),
        `${t('No puedes subir más de')} ${maxFiles} ${maxFiles > 1 ? t('archivos') : t('archivo')}`,
        'warning'
      )
    ...
    acceptedFiles.forEach(file => {
      ...
      const textoExtensionesValidas = allowedExtensions.join(', ')

      // Verificar si el tipo de archivo
      if (!allowedExtensions.includes(fileExtension.toLowerCase()) && archivosAceptados != '*') {
        Swal.fire({
          icon: 'warning',
          title: t('Advertencia'),
          text: `${t('Solo se permiten archivos con extensión')} ${textoExtensionesValidas}`,
          customClass: {
      ...
      if (existingFileIndex !== -1) {
        Swal.fire('Advertencia', `${t('El archivo')} ${file.name} ${t('ya existe')}`, 'question')
      } else {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (file, event) => {
      validacionArchivos(file)
    },
    maxSize: maxSize
  })

  const validacionArchivos = filesValidate => {
    const updatedFiles = [...files]

    // Verificar si la cantidad de archivos existentes más los archivos aceptados supera el límite máximo
    if (updatedFiles.length + filesValidate.length > maxFiles) {
      Swal.fire({
        title: 'Advertencia',
        text: `${t('No puedes subir más de')} ${maxFiles} ${maxFiles > 1 ? t('archivos') : t('archivo')}`,
        icon: 'warning',
        customClass: {
          container: 'my-sweetalert-container'
        }
    ...
    filesValidate.forEach(obj => {
      ...
      // Verificar si el tamaño es el aceptado
      if (file.size > maxSize) {
        Swal.fire({
          icon: 'warning',
          title: t('Advertencia'),
          text: `${t('Solo se permiten archivos con tamaño maximo de')} ${parseInt(maxSize / 1048576)} Mb`,
          customClass: {
            container: 'my-sweetalert-container'
          }
        })
  ...
  const fileList = files.map(file => (
    <Grid item key={file.name} xs={12} sm={6} md={4} lg={files.length === 1 ? 6 : 3}>
      <Card
        className='hovered'
        style={{
      ...
      >
        {file.type === 'application/pdf' ? ( // Válida si el tipo de archivo es .pdf
          <>
            {/* Imagen que muestra de tipo banner */}
            <CardMedia
              component='img'
              alt='pdf'
              height='120'
              image='/images/misc/pdf_banner.png'
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
      <Button id='removeAllFilesBtn' sx={{ display: 'none' }} variant='outlined' onClick={handleRemoveAllFiles}>
        {t('Eliminar todos')}
      </Button>
      <div
        style={{
      ...
      >
        {showDropZone ? (
          <div // Estilo de la caja de arrastre y carga de archivos
            {...getRootProps({ className: `${isDragActive ? 'dropzone' : ''}` })}
            style={{
              padding: '20px',
              borderRadius: '5px',
              textAlign: 'center',
          ...
          >
            ...
            <Grid style={{ marginTop: '10px' }}>
              <Typography color='textSecondary'>
                {t('Arrastre los archivos aquí o haga clic para navegar en su dispositivo')}
              </Typography>
              <Typography color='textSecondary'>{mensaje}</Typography>
              <Typography color='textSecondary'>
                {t('Máximo')} {maxFiles} {maxFiles > 1 ? t('archivos') : t('archivo')}
              </Typography>
              <Typography color='textSecondary'>
                {t('Peso máximo')} {parseInt(maxSize / 1048576)} Mb
              </Typography>
            </Grid>
          </div>
        ) : null}
          ...
          <>
            ...
            >
              <Typography variant='body1' color='textSecondary'>
                {t('Archivos cargados')}: {files.length}
              </Typography>
                ...
                >
                  <Button
                    style={{
                      ...
                      display: files.length < maxFiles ? 'flex' : 'none'
                    }}
                    variant='contained'
                    skin='light'
                    color='success'
      ...
      {/* Modal que permite visualizar el pdf */}
      <Modal open={openPdfModal} onClose={handleClosePdfModal} closeAfterTransition>
        <Fade in={openPdfModal}>
          <div // Estilo que se le da a la vista previa de imagen
            onClick={handleClosePdfModal} // Agregar el evento de clic para cerrar la vista previa
            style={{
              position: 'fixed',
              top: '0',
              left: '0',
