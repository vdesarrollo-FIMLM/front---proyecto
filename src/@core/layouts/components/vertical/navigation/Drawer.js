...
const Drawer = props => {
  ...
  // ** Vars
  const { mode, navCollapsed, themeColor, appBarBlur } = settings
  let flag = true
  ...
  const drawerColors = () => {
    if (mode === 'semi-dark') {
      ...
      if (themeColor == 'home_espiritual_hnocesar') {
        return {
          background: `linear-gradient(to bottom, ${theme.palette[themeColor].dark}, ${theme.palette[themeColor].light})`,
          color: '#001343 !important',
          '& .MuiTypography-root, & .MuiListItemButton-root, & .MuiListItemButton-root.active .MuiTypography-root': {
            color: '#001343 !important'
          },
          '& .MuiListItemIcon-root, & .MuiListItemButton-root.active .MuiListItemIcon-root': {
            color: '#001343 !important'
          }
        }
      } else {
