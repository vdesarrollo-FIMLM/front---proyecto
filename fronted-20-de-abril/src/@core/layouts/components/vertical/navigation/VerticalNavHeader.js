...
const VerticalNavHeader = props => {
  ...
  const logo = {
    localhost: '/login/logo.png'

    // 'develop.domain.org': '/images/apple-touch-icon.png',
    // 'beta.domain.org': '/images/apple-touch-icon.png',
    // 'www.domain.org': '/login/apple-touch-icon.png',
    // 'www.app': '/login/apple-touch-icon.png'
  }
  ...
  return (
    <MenuHeaderWrapper className='nav-header' sx={{ pl: menuHeaderPaddingLeft() }}>
      {userNavMenuBranding ? (
        ...
        <StyledLink href='/' sx={{ width: '500px', ml: navCollapsed ? -2 : 0, mr: navCollapsed ? 1 : 0 }}>
          <img
            src={navCollapsed ? '/images/logoFimlm.png' : '/images/frase-aent.png'}
            width={navCollapsed ? '55px' : '180px'}
            alt='Logo APP'
          />
        </StyledLink>
      )}
